...
public class DutyRosterApp {
        ...
        if(ans.equals("y") || ans.equals("Y"))
        {
            while(true) {
                try {
                    ...
                    System.out.println("读取文件有错误！请选择其他正确文件！");
                    continue;
                }
                break;
            }
            ...
            System.out.println("------------------");
            getTime();
            dutyIntervalSet = new DutyIntervalSet<>(0, endTime.toLong() - startTime.toLong());
    ...
    private static void addNewRecord() {
        ...
        while(true) {
            ...
            try {
                for (Employee e : employees) {
                    if (e.getName().equals(duty_name))
                        dutyIntervalSet.insert(st, et, e);
                }
    ...
    private static boolean checkTable(){
        long x = 0;
        if (dutyIntervalSet.checkNoBlank()) {
            return true;
        }
        System.out.println("当前排班时间未满!");
        System.out.print("未安排时间：");
        for(Interval i:dutyIntervalSet.getBlank()) {
    ...
    private static void deleteRecord(){
        ...
        while(true) {
            ...
            for (Employee e : dutyIntervalSet.labels()) {
                if (e.getName().equals(employee_name)) {
                    flag = true;
                    dutyIntervalSet.remove(e);
                    System.out.println("删除成功！");
                    return;
                }
            }
    ...
    private static void printTable(){
        ...
        if(sorting() != null) {
            List<Employee> s = sorting();
            for (Employee employee : s) {
                num = 0;
                for (long i = dutyIntervalSet.start(employee); i <= dutyIntervalSet.end(employee); i++) {
                    if(i != 0 && num == 0) {
                        num++;
                    }
                    else {
                        print_table(startTime.add(i) + "", employee.getName() + "", employee.getJob() + "", employee.getPhoneNumber() + "");
                        num++;
                    }
                }
            }
     ...
     * 根据值班时间给员工排序
     */
    private static List<Employee> sorting() {
        if(endTime != null && startTime != null) {
            IntervalSet<Employee> s = new DutyIntervalSet<>(0, endTime.toLong() - startTime.toLong());
            if (dutyIntervalSet.labels() != null) {
                List<Employee> list = new ArrayList<>(dutyIntervalSet.labels());
                list.sort(Comparator.comparingInt(o -> (int) dutyIntervalSet.start(o)));
                return list;
            }
        }
        return null;
                ...
                for (Employee emp : employeeArray) {
                    if (emp.getName().equals(employeeName)) {
                        ...
                        if(st.toLong() - startTime.toLong() == 0) {
                            dutyIntervalSet.insert(st.toLong() - startTime.toLong(), et.toLong() - startTime.toLong(), emp);
                        }
                        else {
                            dutyIntervalSet.insert(st.toLong() - startTime.toLong() - 1, et.toLong() - startTime.toLong(), emp);
                        }
                    }
                }
            }
    ...
    private static void autoArranging(){
        ...
        do {
            for (Employee e : employees) {
                ...
                {
                    dutyIntervalSet.insert(st, endTime.toLong() - startTime.toLong(),e);
                    flag = true;
                    break;
                }
                num++;
                ...
                dutyIntervalSet.insert(st, temp, e);
                st = temp;
                if (temp == endTime.toLong() - startTime.toLong()) {
                    flag = true;
                    break;
